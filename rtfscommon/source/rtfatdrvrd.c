...
#endif
#if (INCLUDE_ASYNC_FATLOAD)       /* load syncronously */
     pdr->drive_state.fat_scan_complete=TRUE;
#endif
...
{
#if (INCLUDE_RTFS_PROPLUS||INCLUDE_ASYNC_FATLOAD) /* Proplus set up async check_freespace(DDRIVE *pdr) */
...
int pc_scan_fat_cs(byte *path, int use_charset)
#else
int pc_scan_fat(byte *path)
#endif
{
DDRIVE *pdr;
BOOLEAN ret_val;
...
#if (INCLUDE_RTFS_PROPLUS||INCLUDE_ASYNC_FATLOAD) /* ProPlus - breakout, but first prepare to reenter asynchronous check_freespace */
